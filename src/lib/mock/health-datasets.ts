...
function ds(
  ...
  return {
    ...
    slug,
    title,
    description: opts.description ?? `${title} — Niger State GeoHealth Portal dataset.`,
    ...
    citation: opts.citation ?? `${title}. Niger State GeoHealth Portal. NSPHCDA, ${new Date().getFullYear()}.`,
    dataType: opts.dataType ?? (opts.formats.includes("GeoJSON") ? "spatial" : "attribute"),
    source: opts.source ?? "DHIS2 / NHMIS",
    portalSource: "Niger State GeoHealth Portal",
